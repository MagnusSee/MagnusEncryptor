...
window.title('Magnus Decryptor') # Title Window
...
def tog():
	...
	if len(key) != 32: # Check if key doesn't have 32 digits
		box.showerror('Magnus Decryptor', 'Key Must Have 32 Digits Exactly')
	else:
		...
		try:
			...
			encoded_text = cipher_suite.decrypt(bytes(message, 'utf-8')) # Encrypt the string

			label_result.delete('1.0', 'end') # Delete any old text

			label_result.insert(1.0, 'Decrypted Text: ' + encoded_text.decode('utf-8')) # Save Final Result To Label
			label_result.config(state = 'disabled') # configure textbox to readonly
		except:
			box.showerror('Magnus Decryptor', 'Key or Message Invalid') #If any error occurs show this


decrypt_button = Button(window, text = 'Decrypt', command = tog) # Create Decrypt Buttton
...
decrypt_button.pack(side = TOP, pady = 5)
